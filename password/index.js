...
function handleBcryptSaltRounds(saltRounds = 10){
    ...
    if(saltRoundsType !== 'number') {
        throw new Error(`Parameter saltRounds must be of type number. The provided parameter is of type - ${saltRoundsType}`);
    }
    else if(saltRounds > 15) {
        log('Be careful with using too many salt rounds. Doing so may dramatically slow down hashing.')
    }
    return saltRounds
}
